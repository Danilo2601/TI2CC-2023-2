...
public class JogoService {
	...
	public void makeForm(int tipo, Jogo jogo, int orderBy) {
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			String action = "/jogo/";
			String name, publisher, categoria,nome, buttonLabel;
			if (tipo == FORM_INSERT){
				action += "insert";
				name = "Inserir Jogo";
				nome = "Nome do jogo";
				categoria= "FPS/MOBA/ RPG...";
				publisher= "Actvision/RIOT Games/EA...";
				buttonLabel = "Inserir";
			} else {
			...
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>&nbsp;Descrição: <input class=\"input--register\" type=\"text\" name=\"nome\" value=\""+ nome +"\"></td>";
			umJogo += "\t\t\t<td>Preco: <input class=\"input--register\" type=\"text\" name=\"preco\" value=\""+ jogo.getPreco() +"\"></td>";
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>Categoria: <input class=\"input--register\" type=\"text\" name=\"categoria\" value=\""+ categoria +"\"></td>";
			umJogo += "\t\t\t<td>&nbsp;Publisher: <input class=\"input--register\" type=\"text\" name=\"publisher\" value=\""+ publisher + "\"></td>";
			...
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>Categoria: "+ jogo.getCategoria() +"</td>";
			umJogo += "\t\t\t<td>&nbsp;Publisher: "+ jogo.getPublisher() + "</td>";
			umJogo += "\t\t\t<td>&nbsp;</td>";
			umJogo += "\t\t</tr>";
			umJogo += "\t</table>";		
		} else {
	...
	public Object insert(Request request, Response response) {
		...
		Jogo jogo = new Jogo(-1, preco, nome, categoria, publisher);
	...
	public Object update(Request request, Response response) {
        ...
        if (jogo != null) {
        	...
        	jogo.setCategoria(request.queryParams("categoria"));
        	jogo.setPublisher(request.queryParams("publisher"));
        	jogoDAO.update(jogo);
        	response.status(200); // success
